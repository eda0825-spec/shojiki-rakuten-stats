...
出力 (JSON、これだけ):
{
  "summary_ja": {
    ...
    "narrative": "全体の状況を 2-3 文 (100-200字)。レビュー件数だけでなく返品・不具合Issueも織り込む",
    "key_findings": [
...
6. finding には「件数の内訳 (返品N件・不具合N件・改善要望N件など)」と「具体的に何が言われているか」を入れる
...
出力 (JSON、これだけ):
{
  "summary_ja": {
    ...
    "actions": [
      "工場が今すぐ取れる具体的アクション 1 (40字以内)",
      "工場が今すぐ取れる具体的アクション 2 (40字以内)"
    ]
  },
  "summary_zh": { 同上の中国語訳 }
}
...
- データが 0 件の場合: narrative = "現時点で報告された{タイプ}はありません。" / actions = []
- データが 1 件しかない場合でも、その 1 件の具体的内容に踏み込む
- "actions" は「検討する」「考慮する」など曖昧動詞 NG。「再設計する」「強化する」「変更する」など具体動詞
- JSON 以外何も出力しない
"""
